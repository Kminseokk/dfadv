
#### 

server_list_data = {'rows': [{'serverId': 'cain', 'serverName': '카인'}, 
                             {'serverId': 'diregie', 'serverName': '디레지에'}, 
                             {'serverId': 'siroco', 'serverName': '시로코'}, 
                             {'serverId': 'prey', 'serverName': '프레이'}, 
                             {'serverId': 'casillas', 'serverName': '카시야스'}, 
                             {'serverId': 'hilder', 'serverName': '힐더'}, 
                             {'serverId': 'anton', 'serverName': '안톤'}, 
                             {'serverId': 'bakal', 'serverName': '바칼'}]}

Custom_epic_list = ['숲속의 마녀', '블루 베릴', '엔트 정령', '딥 다이버', '블루 파이렛']

awakening_title_name_list = {'2023 시즌3', '진정한 각성', '소년의 기억을', '흰 구름 감시자', '자신만의'}

Queen_creature_list = {'폭룡왕 바칼', '순백의 나비', '페리아', '태초의 세리아', '차원의 마검사', '신부 세리아'}

fashionista_name_list = {'패셔니스타', '대마법사', '제네시스', '공포를 머금은', '페스타', '축복의 연인'}

buffer_list = {'眞 뮤즈', '眞 크루세이더', '眞 인챈트리스'}

class_data = {
    "rows" : [ {
            "jobId" : "41f1cdc2ff58bb5fdc287be0db2a8df3",
            "jobName" : "귀검사(남)",
            "rows" : [ {
            "jobGrowId" : "df3870efe8e8754011cd12fa03cd275f",
            "jobGrowName" : "웨펀마스터",
            "next" : {
                "jobGrowId" : "4ec01f4ae3728c080f28a72213b6df10",
                "jobGrowName" : "검성",
                "next" : {
                "jobGrowId" : "80ec67d0356defa46a989914caca5820",
                "jobGrowName" : "검신",
                "next" : {
                    "jobGrowId" : "37495b941da3b1661bc900e68ef3b2c6",
                    "jobGrowName" : "眞 웨펀마스터"
                }
                }
            }
            }, {
            "jobGrowId" : "1ea78ae210f681a799feb4403a5c1e85",
            "jobGrowName" : "소울브링어",
            "next" : {
                "jobGrowId" : "981d675414f83b926db5f54e5347eec0",
                "jobGrowName" : "소울테이커",
                "next" : {
                "jobGrowId" : "8022a99e29b5e4845985fe1d79a233f3",
                "jobGrowName" : "다크로드",
                "next" : {
                    "jobGrowId" : "618326026de1a1f1cfba5dbd0b8396e7",
                    "jobGrowName" : "眞 소울브링어"
                }
                }
            }
            }, {
            "jobGrowId" : "a9a4ef4552d46e39cf6c874a51126410",
            "jobGrowName" : "버서커",
            "next" : {
                "jobGrowId" : "460822c3780a9bc3fbe9485cc89c44a4",
                "jobGrowName" : "헬벤터",
                "next" : {
                "jobGrowId" : "15ce2751ab9c8302f39636cdb7c3dcfa",
                "jobGrowName" : "블러드 이블",
                "next" : {
                    "jobGrowId" : "6d459bc74ba73ee4fe5cdc4655400193",
                    "jobGrowName" : "眞 버서커"
                }
                }
            }
            }, {
            "jobGrowId" : "4a1459a4fa3c7f59b6da2e43382ed0b9",
            "jobGrowName" : "아수라",
            "next" : {
                "jobGrowId" : "faf8fc6f0cd4c0f84f0035e23bd4fa19",
                "jobGrowName" : "대암흑천",
                "next" : {
                "jobGrowId" : "64bbd751c0fcae0e74758b871a2296c1",
                "jobGrowName" : "인다라천",
                "next" : {
                    "jobGrowId" : "c9b492038ee3ca8d27d7004cf58d59f3",
                    "jobGrowName" : "眞 아수라"
                }
                }
            }
            }, {
            "jobGrowId" : "a59ba19824dc3292b6075e29b3862ad3",
            "jobGrowName" : "검귀",
            "next" : {
                "jobGrowId" : "a427ec3e94087c64cb4144379c3a0d59",
                "jobGrowName" : "야차",
                "next" : {
                "jobGrowId" : "3ff5b661925ac59d1ca290b05ca914cc",
                "jobGrowName" : "악귀나찰",
                "next" : {
                    "jobGrowId" : "92da05ec93fb43406e193ffb9a2a629b",
                    "jobGrowName" : "眞 검귀"
                }
                }
            }
            } ]
        }, {
            "jobId" : "a7a059ebe9e6054c0644b40ef316d6e9",
            "jobName" : "격투가(여)",
            "rows" : [ {
            "jobGrowId" : "df3870efe8e8754011cd12fa03cd275f",
            "jobGrowName" : "넨마스터",
            "next" : {
                "jobGrowId" : "4ec01f4ae3728c080f28a72213b6df10",
                "jobGrowName" : "백화요란",
                "next" : {
                "jobGrowId" : "80ec67d0356defa46a989914caca5820",
                "jobGrowName" : "염제 폐월수화",
                "next" : {
                    "jobGrowId" : "37495b941da3b1661bc900e68ef3b2c6",
                    "jobGrowName" : "眞 넨마스터"
                }
                }
            }
            }, {
            "jobGrowId" : "1ea78ae210f681a799feb4403a5c1e85",
            "jobGrowName" : "스트라이커",
            "next" : {
                "jobGrowId" : "981d675414f83b926db5f54e5347eec0",
                "jobGrowName" : "챔피언",
                "next" : {
                "jobGrowId" : "8022a99e29b5e4845985fe1d79a233f3",
                "jobGrowName" : "카이저",
                "next" : {
                    "jobGrowId" : "618326026de1a1f1cfba5dbd0b8396e7",
                    "jobGrowName" : "眞 스트라이커"
                }
                }
            }
            }, {
            "jobGrowId" : "a9a4ef4552d46e39cf6c874a51126410",
            "jobGrowName" : "스트리트파이터",
            "next" : {
                "jobGrowId" : "460822c3780a9bc3fbe9485cc89c44a4",
                "jobGrowName" : "독왕",
                "next" : {
                "jobGrowId" : "15ce2751ab9c8302f39636cdb7c3dcfa",
                "jobGrowName" : "용독문주",
                "next" : {
                    "jobGrowId" : "6d459bc74ba73ee4fe5cdc4655400193",
                    "jobGrowName" : "眞 스트리트파이터"
                }
                }
            }
            }, {
            "jobGrowId" : "4a1459a4fa3c7f59b6da2e43382ed0b9",
            "jobGrowName" : "그래플러",
            "next" : {
                "jobGrowId" : "faf8fc6f0cd4c0f84f0035e23bd4fa19",
                "jobGrowName" : "토네이도",
                "next" : {
                "jobGrowId" : "64bbd751c0fcae0e74758b871a2296c1",
                "jobGrowName" : "얼티밋 디바",
                "next" : {
                    "jobGrowId" : "c9b492038ee3ca8d27d7004cf58d59f3",
                    "jobGrowName" : "眞 그래플러"
                }
                }
            }
            } ]
        }, {
            "jobId" : "afdf3b989339de478e85b614d274d1ef",
            "jobName" : "거너(남)",
            "rows" : [ {
            "jobGrowId" : "df3870efe8e8754011cd12fa03cd275f",
            "jobGrowName" : "레인저",
            "next" : {
                "jobGrowId" : "4ec01f4ae3728c080f28a72213b6df10",
                "jobGrowName" : "데스페라도",
                "next" : {
                "jobGrowId" : "80ec67d0356defa46a989914caca5820",
                "jobGrowName" : "레이븐",
                "next" : {
                    "jobGrowId" : "37495b941da3b1661bc900e68ef3b2c6",
                    "jobGrowName" : "眞 레인저"
                }
                }
            }
            }, {
            "jobGrowId" : "1ea78ae210f681a799feb4403a5c1e85",
            "jobGrowName" : "런처",
            "next" : {
                "jobGrowId" : "981d675414f83b926db5f54e5347eec0",
                "jobGrowName" : "블래스터",
                "next" : {
                "jobGrowId" : "8022a99e29b5e4845985fe1d79a233f3",
                "jobGrowName" : "디스트로이어",
                "next" : {
                    "jobGrowId" : "618326026de1a1f1cfba5dbd0b8396e7",
                    "jobGrowName" : "眞 런처"
                }
                }
            }
            }, {
            "jobGrowId" : "a9a4ef4552d46e39cf6c874a51126410",
            "jobGrowName" : "메카닉",
            "next" : {
                "jobGrowId" : "460822c3780a9bc3fbe9485cc89c44a4",
                "jobGrowName" : "마이스터",
                "next" : {
                "jobGrowId" : "15ce2751ab9c8302f39636cdb7c3dcfa",
                "jobGrowName" : "프라임",
                "next" : {
                    "jobGrowId" : "6d459bc74ba73ee4fe5cdc4655400193",
                    "jobGrowName" : "眞 메카닉"
                }
                }
            }
            }, {
            "jobGrowId" : "4a1459a4fa3c7f59b6da2e43382ed0b9",
            "jobGrowName" : "스핏파이어",
            "next" : {
                "jobGrowId" : "faf8fc6f0cd4c0f84f0035e23bd4fa19",
                "jobGrowName" : "제네럴",
                "next" : {
                "jobGrowId" : "64bbd751c0fcae0e74758b871a2296c1",
                "jobGrowName" : "커맨더",
                "next" : {
                    "jobGrowId" : "c9b492038ee3ca8d27d7004cf58d59f3",
                    "jobGrowName" : "眞 스핏파이어"
                }
                }
            }
            }, {
            "jobGrowId" : "a59ba19824dc3292b6075e29b3862ad3",
            "jobGrowName" : "어썰트",
            "next" : {
                "jobGrowId" : "a427ec3e94087c64cb4144379c3a0d59",
                "jobGrowName" : "인트루더",
                "next" : {
                "jobGrowId" : "3ff5b661925ac59d1ca290b05ca914cc",
                "jobGrowName" : "엑스마키나",
                "next" : {
                    "jobGrowId" : "92da05ec93fb43406e193ffb9a2a629b",
                    "jobGrowName" : "眞 어썰트"
                }
                }
            }
            } ]
        }, {
            "jobId" : "3909d0b188e9c95311399f776e331da5",
            "jobName" : "마법사(여)",
            "rows" : [ {
            "jobGrowId" : "df3870efe8e8754011cd12fa03cd275f",
            "jobGrowName" : "엘레멘탈마스터",
            "next" : {
                "jobGrowId" : "4ec01f4ae3728c080f28a72213b6df10",
                "jobGrowName" : "아크메이지",
                "next" : {
                "jobGrowId" : "80ec67d0356defa46a989914caca5820",
                "jobGrowName" : "오버마인드",
                "next" : {
                    "jobGrowId" : "37495b941da3b1661bc900e68ef3b2c6",
                    "jobGrowName" : "眞 엘레멘탈마스터"
                }
                }
            }
            }, {
            "jobGrowId" : "1ea78ae210f681a799feb4403a5c1e85",
            "jobGrowName" : "소환사",
            "next" : {
                "jobGrowId" : "981d675414f83b926db5f54e5347eec0",
                "jobGrowName" : "문 엠프레스",
                "next" : {
                "jobGrowId" : "8022a99e29b5e4845985fe1d79a233f3",
                "jobGrowName" : "이클립스",
                "next" : {
                    "jobGrowId" : "618326026de1a1f1cfba5dbd0b8396e7",
                    "jobGrowName" : "眞 소환사"
                }
                }
            }
            }, {
            "jobGrowId" : "a9a4ef4552d46e39cf6c874a51126410",
            "jobGrowName" : "배틀메이지",
            "next" : {
                "jobGrowId" : "460822c3780a9bc3fbe9485cc89c44a4",
                "jobGrowName" : "벨라트릭스",
                "next" : {
                "jobGrowId" : "15ce2751ab9c8302f39636cdb7c3dcfa",
                "jobGrowName" : "아슈타르테",
                "next" : {
                    "jobGrowId" : "6d459bc74ba73ee4fe5cdc4655400193",
                    "jobGrowName" : "眞 배틀메이지"
                }
                }
            }
            }, {
            "jobGrowId" : "4a1459a4fa3c7f59b6da2e43382ed0b9",
            "jobGrowName" : "마도학자",
            "next" : {
                "jobGrowId" : "faf8fc6f0cd4c0f84f0035e23bd4fa19",
                "jobGrowName" : "트릭스터",
                "next" : {
                "jobGrowId" : "64bbd751c0fcae0e74758b871a2296c1",
                "jobGrowName" : "지니위즈",
                "next" : {
                    "jobGrowId" : "c9b492038ee3ca8d27d7004cf58d59f3",
                    "jobGrowName" : "眞 마도학자"
                }
                }
            }
            }, {
            "jobGrowId" : "a59ba19824dc3292b6075e29b3862ad3",
            "jobGrowName" : "인챈트리스",
            "next" : {
                "jobGrowId" : "a427ec3e94087c64cb4144379c3a0d59",
                "jobGrowName" : "블랙 메이든",
                "next" : {
                "jobGrowId" : "3ff5b661925ac59d1ca290b05ca914cc",
                "jobGrowName" : "헤카테",
                "next" : {
                    "jobGrowId" : "92da05ec93fb43406e193ffb9a2a629b",
                    "jobGrowName" : "眞 인챈트리스"
                }
                }
            }
            } ]
        }, {
            "jobId" : "f6a4ad30555b99b499c07835f87ce522",
            "jobName" : "프리스트(남)",
            "rows" : [ {
            "jobGrowId" : "df3870efe8e8754011cd12fa03cd275f",
            "jobGrowName" : "크루세이더",
            "next" : {
                "jobGrowId" : "4ec01f4ae3728c080f28a72213b6df10",
                "jobGrowName" : "홀리오더",
                "next" : {
                "jobGrowId" : "80ec67d0356defa46a989914caca5820",
                "jobGrowName" : "세인트",
                "next" : {
                    "jobGrowId" : "37495b941da3b1661bc900e68ef3b2c6",
                    "jobGrowName" : "眞 크루세이더"
                }
                }
            }
            }, {
            "jobGrowId" : "1ea78ae210f681a799feb4403a5c1e85",
            "jobGrowName" : "인파이터",
            "next" : {
                "jobGrowId" : "981d675414f83b926db5f54e5347eec0",
                "jobGrowName" : "갓핸드",
                "next" : {
                "jobGrowId" : "8022a99e29b5e4845985fe1d79a233f3",
                "jobGrowName" : "저스티스",
                "next" : {
                    "jobGrowId" : "618326026de1a1f1cfba5dbd0b8396e7",
                    "jobGrowName" : "眞 인파이터"
                }
                }
            }
            }, {
            "jobGrowId" : "a9a4ef4552d46e39cf6c874a51126410",
            "jobGrowName" : "퇴마사",
            "next" : {
                "jobGrowId" : "460822c3780a9bc3fbe9485cc89c44a4",
                "jobGrowName" : "용투사",
                "next" : {
                "jobGrowId" : "15ce2751ab9c8302f39636cdb7c3dcfa",
                "jobGrowName" : "태을선인",
                "next" : {
                    "jobGrowId" : "6d459bc74ba73ee4fe5cdc4655400193",
                    "jobGrowName" : "眞 퇴마사"
                }
                }
            }
            }, {
            "jobGrowId" : "4a1459a4fa3c7f59b6da2e43382ed0b9",
            "jobGrowName" : "어벤저",
            "next" : {
                "jobGrowId" : "faf8fc6f0cd4c0f84f0035e23bd4fa19",
                "jobGrowName" : "둠스가디언",
                "next" : {
                "jobGrowId" : "64bbd751c0fcae0e74758b871a2296c1",
                "jobGrowName" : "이모탈",
                "next" : {
                    "jobGrowId" : "c9b492038ee3ca8d27d7004cf58d59f3",
                    "jobGrowName" : "眞 어벤저"
                }
                }
            }
            } ]
        }, {
            "jobId" : "944b9aab492c15a8474f96947ceeb9e4",
            "jobName" : "거너(여)",
            "rows" : [ {
            "jobGrowId" : "df3870efe8e8754011cd12fa03cd275f",
            "jobGrowName" : "레인저",
            "next" : {
                "jobGrowId" : "4ec01f4ae3728c080f28a72213b6df10",
                "jobGrowName" : "블러디아",
                "next" : {
                "jobGrowId" : "80ec67d0356defa46a989914caca5820",
                "jobGrowName" : "크림슨로제",
                "next" : {
                    "jobGrowId" : "37495b941da3b1661bc900e68ef3b2c6",
                    "jobGrowName" : "眞 레인저"
                }
                }
            }
            }, {
            "jobGrowId" : "1ea78ae210f681a799feb4403a5c1e85",
            "jobGrowName" : "런처",
            "next" : {
                "jobGrowId" : "981d675414f83b926db5f54e5347eec0",
                "jobGrowName" : "헤비배럴",
                "next" : {
                "jobGrowId" : "8022a99e29b5e4845985fe1d79a233f3",
                "jobGrowName" : "스톰트루퍼",
                "next" : {
                    "jobGrowId" : "618326026de1a1f1cfba5dbd0b8396e7",
                    "jobGrowName" : "眞 런처"
                }
                }
            }
            }, {
            "jobGrowId" : "a9a4ef4552d46e39cf6c874a51126410",
            "jobGrowName" : "메카닉",
            "next" : {
                "jobGrowId" : "460822c3780a9bc3fbe9485cc89c44a4",
                "jobGrowName" : "메탈하트",
                "next" : {
                "jobGrowId" : "15ce2751ab9c8302f39636cdb7c3dcfa",
                "jobGrowName" : "옵티머스",
                "next" : {
                    "jobGrowId" : "6d459bc74ba73ee4fe5cdc4655400193",
                    "jobGrowName" : "眞 메카닉"
                }
                }
            }
            }, {
            "jobGrowId" : "4a1459a4fa3c7f59b6da2e43382ed0b9",
            "jobGrowName" : "스핏파이어",
            "next" : {
                "jobGrowId" : "faf8fc6f0cd4c0f84f0035e23bd4fa19",
                "jobGrowName" : "발키리",
                "next" : {
                "jobGrowId" : "64bbd751c0fcae0e74758b871a2296c1",
                "jobGrowName" : "프레이야",
                "next" : {
                    "jobGrowId" : "c9b492038ee3ca8d27d7004cf58d59f3",
                    "jobGrowName" : "眞 스핏파이어"
                }
                }
            }
            } ]
        }, {
            "jobId" : "ddc49e9ad1ff72a00b53c6cff5b1e920",
            "jobName" : "도적",
            "rows" : [ {
            "jobGrowId" : "df3870efe8e8754011cd12fa03cd275f",
            "jobGrowName" : "로그",
            "next" : {
                "jobGrowId" : "4ec01f4ae3728c080f28a72213b6df10",
                "jobGrowName" : "실버문",
                "next" : {
                "jobGrowId" : "80ec67d0356defa46a989914caca5820",
                "jobGrowName" : "알키오네",
                "next" : {
                    "jobGrowId" : "37495b941da3b1661bc900e68ef3b2c6",
                    "jobGrowName" : "眞 로그"
                }
                }
            }
            }, {
            "jobGrowId" : "1ea78ae210f681a799feb4403a5c1e85",
            "jobGrowName" : "사령술사",
            "next" : {
                "jobGrowId" : "981d675414f83b926db5f54e5347eec0",
                "jobGrowName" : "데스브링어",
                "next" : {
                "jobGrowId" : "8022a99e29b5e4845985fe1d79a233f3",
                "jobGrowName" : "타나토스",
                "next" : {
                    "jobGrowId" : "618326026de1a1f1cfba5dbd0b8396e7",
                    "jobGrowName" : "眞 사령술사"
                }
                }
            }
            }, {
            "jobGrowId" : "a9a4ef4552d46e39cf6c874a51126410",
            "jobGrowName" : "쿠노이치",
            "next" : {
                "jobGrowId" : "460822c3780a9bc3fbe9485cc89c44a4",
                "jobGrowName" : "이즈나비",
                "next" : {
                "jobGrowId" : "15ce2751ab9c8302f39636cdb7c3dcfa",
                "jobGrowName" : "시라누이",
                "next" : {
                    "jobGrowId" : "6d459bc74ba73ee4fe5cdc4655400193",
                    "jobGrowName" : "眞 쿠노이치"
                }
                }
            }
            }, {
            "jobGrowId" : "4a1459a4fa3c7f59b6da2e43382ed0b9",
            "jobGrowName" : "섀도우댄서",
            "next" : {
                "jobGrowId" : "faf8fc6f0cd4c0f84f0035e23bd4fa19",
                "jobGrowName" : "나이트메어",
                "next" : {
                "jobGrowId" : "64bbd751c0fcae0e74758b871a2296c1",
                "jobGrowName" : "그림리퍼",
                "next" : {
                    "jobGrowId" : "c9b492038ee3ca8d27d7004cf58d59f3",
                    "jobGrowName" : "眞 섀도우댄서"
                }
                }
            }
            } ]
        }, {
            "jobId" : "ca0f0e0e9e1d55b5f9955b03d9dd213c",
            "jobName" : "격투가(남)",
            "rows" : [ {
            "jobGrowId" : "df3870efe8e8754011cd12fa03cd275f",
            "jobGrowName" : "넨마스터",
            "next" : {
                "jobGrowId" : "4ec01f4ae3728c080f28a72213b6df10",
                "jobGrowName" : "광호제",
                "next" : {
                "jobGrowId" : "80ec67d0356defa46a989914caca5820",
                "jobGrowName" : "염황 광풍제월",
                "next" : {
                    "jobGrowId" : "37495b941da3b1661bc900e68ef3b2c6",
                    "jobGrowName" : "眞 넨마스터"
                }
                }
            }
            }, {
            "jobGrowId" : "1ea78ae210f681a799feb4403a5c1e85",
            "jobGrowName" : "스트라이커",
            "next" : {
                "jobGrowId" : "981d675414f83b926db5f54e5347eec0",
                "jobGrowName" : "무극",
                "next" : {
                "jobGrowId" : "8022a99e29b5e4845985fe1d79a233f3",
                "jobGrowName" : "패황",
                "next" : {
                    "jobGrowId" : "618326026de1a1f1cfba5dbd0b8396e7",
                    "jobGrowName" : "眞 스트라이커"
                }
                }
            }
            }, {
            "jobGrowId" : "a9a4ef4552d46e39cf6c874a51126410",
            "jobGrowName" : "스트리트파이터",
            "next" : {
                "jobGrowId" : "460822c3780a9bc3fbe9485cc89c44a4",
                "jobGrowName" : "천수나한",
                "next" : {
                "jobGrowId" : "15ce2751ab9c8302f39636cdb7c3dcfa",
                "jobGrowName" : "명왕",
                "next" : {
                    "jobGrowId" : "6d459bc74ba73ee4fe5cdc4655400193",
                    "jobGrowName" : "眞 스트리트파이터"
                }
                }
            }
            }, {
            "jobGrowId" : "4a1459a4fa3c7f59b6da2e43382ed0b9",
            "jobGrowName" : "그래플러",
            "next" : {
                "jobGrowId" : "faf8fc6f0cd4c0f84f0035e23bd4fa19",
                "jobGrowName" : "자이언트",
                "next" : {
                "jobGrowId" : "64bbd751c0fcae0e74758b871a2296c1",
                "jobGrowName" : "그랜드 마스터",
                "next" : {
                    "jobGrowId" : "c9b492038ee3ca8d27d7004cf58d59f3",
                    "jobGrowName" : "眞 그래플러"
                }
                }
            }
            } ]
        }, {
            "jobId" : "a5ccbaf5538981c6ef99b236c0a60b73",
            "jobName" : "마법사(남)",
            "rows" : [ {
            "jobGrowId" : "df3870efe8e8754011cd12fa03cd275f",
            "jobGrowName" : "엘레멘탈 바머",
            "next" : {
                "jobGrowId" : "4ec01f4ae3728c080f28a72213b6df10",
                "jobGrowName" : "워록",
                "next" : {
                "jobGrowId" : "80ec67d0356defa46a989914caca5820",
                "jobGrowName" : "오블리비언",
                "next" : {
                    "jobGrowId" : "37495b941da3b1661bc900e68ef3b2c6",
                    "jobGrowName" : "眞 엘레멘탈 바머"
                }
                }
            }
            }, {
            "jobGrowId" : "1ea78ae210f681a799feb4403a5c1e85",
            "jobGrowName" : "빙결사",
            "next" : {
                "jobGrowId" : "981d675414f83b926db5f54e5347eec0",
                "jobGrowName" : "프로즌하트",
                "next" : {
                "jobGrowId" : "8022a99e29b5e4845985fe1d79a233f3",
                "jobGrowName" : "이터널",
                "next" : {
                    "jobGrowId" : "618326026de1a1f1cfba5dbd0b8396e7",
                    "jobGrowName" : "眞 빙결사"
                }
                }
            }
            }, {
            "jobGrowId" : "a9a4ef4552d46e39cf6c874a51126410",
            "jobGrowName" : "블러드 메이지",
            "next" : {
                "jobGrowId" : "460822c3780a9bc3fbe9485cc89c44a4",
                "jobGrowName" : "뱀파이어",
                "next" : {
                "jobGrowId" : "15ce2751ab9c8302f39636cdb7c3dcfa",
                "jobGrowName" : "뱀파이어 로드",
                "next" : {
                    "jobGrowId" : "6d459bc74ba73ee4fe5cdc4655400193",
                    "jobGrowName" : "眞 블러드 메이지"
                }
                }
            }
            }, {
            "jobGrowId" : "4a1459a4fa3c7f59b6da2e43382ed0b9",
            "jobGrowName" : "스위프트 마스터",
            "next" : {
                "jobGrowId" : "faf8fc6f0cd4c0f84f0035e23bd4fa19",
                "jobGrowName" : "스톰브링어",
                "next" : {
                "jobGrowId" : "64bbd751c0fcae0e74758b871a2296c1",
                "jobGrowName" : "아이올로스",
                "next" : {
                    "jobGrowId" : "c9b492038ee3ca8d27d7004cf58d59f3",
                    "jobGrowName" : "眞 스위프트 마스터"
                }
                }
            }
            }, {
            "jobGrowId" : "a59ba19824dc3292b6075e29b3862ad3",
            "jobGrowName" : "디멘션워커",
            "next" : {
                "jobGrowId" : "a427ec3e94087c64cb4144379c3a0d59",
                "jobGrowName" : "이레귤러",
                "next" : {
                "jobGrowId" : "3ff5b661925ac59d1ca290b05ca914cc",
                "jobGrowName" : "어센션",
                "next" : {
                    "jobGrowId" : "92da05ec93fb43406e193ffb9a2a629b",
                    "jobGrowName" : "眞 디멘션워커"
                }
                }
            }
            } ]
        }, {
            "jobId" : "17e417b31686389eebff6d754c3401ea",
            "jobName" : "다크나이트",
            "rows" : [ {
            "jobGrowId" : "4fdee159d5aa8874a1459861ced676ec",
            "jobGrowName" : "자각1",
            "next" : {
                "jobGrowId" : "da6a126f45370e1a4cbbe8823c6c35be",
                "jobGrowName" : "자각2",
                "next" : {
                "jobGrowId" : "0a49d9c8b5e1358efff324e5cb11d41e",
                "jobGrowName" : "眞 다크나이트"
                }
            }
            } ]
        }, {
            "jobId" : "b522a95d819a5559b775deb9a490e49a",
            "jobName" : "크리에이터",
            "rows" : [ {
            "jobGrowId" : "4fdee159d5aa8874a1459861ced676ec",
            "jobGrowName" : "자각1",
            "next" : {
                "jobGrowId" : "da6a126f45370e1a4cbbe8823c6c35be",
                "jobGrowName" : "자각2",
                "next" : {
                "jobGrowId" : "0a49d9c8b5e1358efff324e5cb11d41e",
                "jobGrowName" : "眞 크리에이터"
                }
            }
            } ]
        }, {
            "jobId" : "1645c45aabb008c98406b3a16447040d",
            "jobName" : "귀검사(여)",
            "rows" : [ {
            "jobGrowId" : "df3870efe8e8754011cd12fa03cd275f",
            "jobGrowName" : "소드마스터",
            "next" : {
                "jobGrowId" : "4ec01f4ae3728c080f28a72213b6df10",
                "jobGrowName" : "노블레스",
                "next" : {
                "jobGrowId" : "80ec67d0356defa46a989914caca5820",
                "jobGrowName" : "마제스티",
                "next" : {
                    "jobGrowId" : "37495b941da3b1661bc900e68ef3b2c6",
                    "jobGrowName" : "眞 소드마스터"
                }
                }
            }
            }, {
            "jobGrowId" : "1ea78ae210f681a799feb4403a5c1e85",
            "jobGrowName" : "다크템플러",
            "next" : {
                "jobGrowId" : "981d675414f83b926db5f54e5347eec0",
                "jobGrowName" : "암제",
                "next" : {
                "jobGrowId" : "8022a99e29b5e4845985fe1d79a233f3",
                "jobGrowName" : "네메시스",
                "next" : {
                    "jobGrowId" : "618326026de1a1f1cfba5dbd0b8396e7",
                    "jobGrowName" : "眞 다크템플러"
                }
                }
            }
            }, {
            "jobGrowId" : "a9a4ef4552d46e39cf6c874a51126410",
            "jobGrowName" : "데몬슬레이어",
            "next" : {
                "jobGrowId" : "460822c3780a9bc3fbe9485cc89c44a4",
                "jobGrowName" : "검마",
                "next" : {
                "jobGrowId" : "15ce2751ab9c8302f39636cdb7c3dcfa",
                "jobGrowName" : "디어사이드",
                "next" : {
                    "jobGrowId" : "6d459bc74ba73ee4fe5cdc4655400193",
                    "jobGrowName" : "眞 데몬슬레이어"
                }
                }
            }
            }, {
            "jobGrowId" : "4a1459a4fa3c7f59b6da2e43382ed0b9",
            "jobGrowName" : "베가본드",
            "next" : {
                "jobGrowId" : "faf8fc6f0cd4c0f84f0035e23bd4fa19",
                "jobGrowName" : "검호",
                "next" : {
                "jobGrowId" : "64bbd751c0fcae0e74758b871a2296c1",
                "jobGrowName" : "검제",
                "next" : {
                    "jobGrowId" : "c9b492038ee3ca8d27d7004cf58d59f3",
                    "jobGrowName" : "眞 베가본드"
                }
                }
            }
            }, {
            "jobGrowId" : "a59ba19824dc3292b6075e29b3862ad3",
            "jobGrowName" : "블레이드",
            "next" : {
                "jobGrowId" : "a427ec3e94087c64cb4144379c3a0d59",
                "jobGrowName" : "리벨리온",
                "next" : {
                "jobGrowId" : "3ff5b661925ac59d1ca290b05ca914cc",
                "jobGrowName" : "벤데타",
                "next" : {
                    "jobGrowId" : "92da05ec93fb43406e193ffb9a2a629b",
                    "jobGrowName" : "眞 블레이드"
                }
                }
            }
            } ]
        }, {
            "jobId" : "0ee8fa5dc525c1a1f23fc6911e921e4a",
            "jobName" : "나이트",
            "rows" : [ {
            "jobGrowId" : "df3870efe8e8754011cd12fa03cd275f",
            "jobGrowName" : "엘븐나이트",
            "next" : {
                "jobGrowId" : "4ec01f4ae3728c080f28a72213b6df10",
                "jobGrowName" : "피스메이커",
                "next" : {
                "jobGrowId" : "80ec67d0356defa46a989914caca5820",
                "jobGrowName" : "가이아",
                "next" : {
                    "jobGrowId" : "37495b941da3b1661bc900e68ef3b2c6",
                    "jobGrowName" : "眞 엘븐나이트"
                }
                }
            }
            }, {
            "jobGrowId" : "1ea78ae210f681a799feb4403a5c1e85",
            "jobGrowName" : "카오스",
            "next" : {
                "jobGrowId" : "981d675414f83b926db5f54e5347eec0",
                "jobGrowName" : "마왕",
                "next" : {
                "jobGrowId" : "8022a99e29b5e4845985fe1d79a233f3",
                "jobGrowName" : "마신",
                "next" : {
                    "jobGrowId" : "618326026de1a1f1cfba5dbd0b8396e7",
                    "jobGrowName" : "眞 카오스"
                }
                }
            }
            }, {
            "jobGrowId" : "a9a4ef4552d46e39cf6c874a51126410",
            "jobGrowName" : "팔라딘",
            "next" : {
                "jobGrowId" : "460822c3780a9bc3fbe9485cc89c44a4",
                "jobGrowName" : "센티넬",
                "next" : {
                "jobGrowId" : "15ce2751ab9c8302f39636cdb7c3dcfa",
                "jobGrowName" : "세이비어",
                "next" : {
                    "jobGrowId" : "6d459bc74ba73ee4fe5cdc4655400193",
                    "jobGrowName" : "眞 팔라딘"
                }
                }
            }
            }, {
            "jobGrowId" : "4a1459a4fa3c7f59b6da2e43382ed0b9",
            "jobGrowName" : "드래곤나이트",
            "next" : {
                "jobGrowId" : "faf8fc6f0cd4c0f84f0035e23bd4fa19",
                "jobGrowName" : "타이런트",
                "next" : {
                "jobGrowId" : "64bbd751c0fcae0e74758b871a2296c1",
                "jobGrowName" : "드레드노트",
                "next" : {
                    "jobGrowId" : "c9b492038ee3ca8d27d7004cf58d59f3",
                    "jobGrowName" : "眞 드래곤나이트"
                }
                }
            }
            } ]
        }, {
            "jobId" : "3deb7be5f01953ac8b1ecaa1e25e0420",
            "jobName" : "마창사",
            "rows" : [ {
            "jobGrowId" : "df3870efe8e8754011cd12fa03cd275f",
            "jobGrowName" : "뱅가드",
            "next" : {
                "jobGrowId" : "4ec01f4ae3728c080f28a72213b6df10",
                "jobGrowName" : "레버넌트",
                "next" : {
                "jobGrowId" : "80ec67d0356defa46a989914caca5820",
                "jobGrowName" : "워로드",
                "next" : {
                    "jobGrowId" : "37495b941da3b1661bc900e68ef3b2c6",
                    "jobGrowName" : "眞 뱅가드"
                }
                }
            }
            }, {
            "jobGrowId" : "1ea78ae210f681a799feb4403a5c1e85",
            "jobGrowName" : "듀얼리스트",
            "next" : {
                "jobGrowId" : "981d675414f83b926db5f54e5347eec0",
                "jobGrowName" : "하이랜더",
                "next" : {
                "jobGrowId" : "8022a99e29b5e4845985fe1d79a233f3",
                "jobGrowName" : "듀란달",
                "next" : {
                    "jobGrowId" : "618326026de1a1f1cfba5dbd0b8396e7",
                    "jobGrowName" : "眞 듀얼리스트"
                }
                }
            }
            }, {
            "jobGrowId" : "a9a4ef4552d46e39cf6c874a51126410",
            "jobGrowName" : "드래고니안 랜서",
            "next" : {
                "jobGrowId" : "460822c3780a9bc3fbe9485cc89c44a4",
                "jobGrowName" : "레비아탄",
                "next" : {
                "jobGrowId" : "15ce2751ab9c8302f39636cdb7c3dcfa",
                "jobGrowName" : "제노사이더",
                "next" : {
                    "jobGrowId" : "6d459bc74ba73ee4fe5cdc4655400193",
                    "jobGrowName" : "眞 드래고니안 랜서"
                }
                }
            }
            }, {
            "jobGrowId" : "4a1459a4fa3c7f59b6da2e43382ed0b9",
            "jobGrowName" : "다크 랜서",
            "next" : {
                "jobGrowId" : "faf8fc6f0cd4c0f84f0035e23bd4fa19",
                "jobGrowName" : "램페이저",
                "next" : {
                "jobGrowId" : "64bbd751c0fcae0e74758b871a2296c1",
                "jobGrowName" : "에레보스",
                "next" : {
                    "jobGrowId" : "c9b492038ee3ca8d27d7004cf58d59f3",
                    "jobGrowName" : "眞 다크 랜서"
                }
                }
            }
            } ]
        }, {
            "jobId" : "0c1b401bb09241570d364420b3ba3fd7",
            "jobName" : "프리스트(여)",
            "rows" : [ {
            "jobGrowId" : "df3870efe8e8754011cd12fa03cd275f",
            "jobGrowName" : "크루세이더",
            "next" : {
                "jobGrowId" : "4ec01f4ae3728c080f28a72213b6df10",
                "jobGrowName" : "에반젤리스트",
                "next" : {
                "jobGrowId" : "80ec67d0356defa46a989914caca5820",
                "jobGrowName" : "세라핌",
                "next" : {
                    "jobGrowId" : "37495b941da3b1661bc900e68ef3b2c6",
                    "jobGrowName" : "眞 크루세이더"
                }
                }
            }
            }, {
            "jobGrowId" : "1ea78ae210f681a799feb4403a5c1e85",
            "jobGrowName" : "이단심판관",
            "next" : {
                "jobGrowId" : "981d675414f83b926db5f54e5347eec0",
                "jobGrowName" : "헬카이트",
                "next" : {
                "jobGrowId" : "8022a99e29b5e4845985fe1d79a233f3",
                "jobGrowName" : "인페르노",
                "next" : {
                    "jobGrowId" : "618326026de1a1f1cfba5dbd0b8396e7",
                    "jobGrowName" : "眞 이단심판관"
                }
                }
            }
            }, {
            "jobGrowId" : "a9a4ef4552d46e39cf6c874a51126410",
            "jobGrowName" : "무녀",
            "next" : {
                "jobGrowId" : "460822c3780a9bc3fbe9485cc89c44a4",
                "jobGrowName" : "신녀",
                "next" : {
                "jobGrowId" : "15ce2751ab9c8302f39636cdb7c3dcfa",
                "jobGrowName" : "천선낭랑",
                "next" : {
                    "jobGrowId" : "6d459bc74ba73ee4fe5cdc4655400193",
                    "jobGrowName" : "眞 무녀"
                }
                }
            }
            }, {
            "jobGrowId" : "4a1459a4fa3c7f59b6da2e43382ed0b9",
            "jobGrowName" : "미스트리스",
            "next" : {
                "jobGrowId" : "faf8fc6f0cd4c0f84f0035e23bd4fa19",
                "jobGrowName" : "신세이어",
                "next" : {
                "jobGrowId" : "64bbd751c0fcae0e74758b871a2296c1",
                "jobGrowName" : "리디머",
                "next" : {
                    "jobGrowId" : "c9b492038ee3ca8d27d7004cf58d59f3",
                    "jobGrowName" : "眞 미스트리스"
                }
                }
            }
            } ]
        }, {
            "jobId" : "986c2b3d72ee0e4a0b7fcfbe786d4e02",
            "jobName" : "총검사",
            "rows" : [ {
            "jobGrowId" : "df3870efe8e8754011cd12fa03cd275f",
            "jobGrowName" : "히트맨",
            "next" : {
                "jobGrowId" : "4ec01f4ae3728c080f28a72213b6df10",
                "jobGrowName" : "빅보스",
                "next" : {
                "jobGrowId" : "80ec67d0356defa46a989914caca5820",
                "jobGrowName" : "갓파더",
                "next" : {
                    "jobGrowId" : "37495b941da3b1661bc900e68ef3b2c6",
                    "jobGrowName" : "眞 히트맨"
                }
                }
            }
            }, {
            "jobGrowId" : "1ea78ae210f681a799feb4403a5c1e85",
            "jobGrowName" : "요원",
            "next" : {
                "jobGrowId" : "981d675414f83b926db5f54e5347eec0",
                "jobGrowName" : "인포서",
                "next" : {
                "jobGrowId" : "8022a99e29b5e4845985fe1d79a233f3",
                "jobGrowName" : "레퀴엠",
                "next" : {
                    "jobGrowId" : "618326026de1a1f1cfba5dbd0b8396e7",
                    "jobGrowName" : "眞 요원"
                }
                }
            }
            }, {
            "jobGrowId" : "a9a4ef4552d46e39cf6c874a51126410",
            "jobGrowName" : "트러블 슈터",
            "next" : {
                "jobGrowId" : "460822c3780a9bc3fbe9485cc89c44a4",
                "jobGrowName" : "와일드 카드",
                "next" : {
                "jobGrowId" : "15ce2751ab9c8302f39636cdb7c3dcfa",
                "jobGrowName" : "언터처블",
                "next" : {
                    "jobGrowId" : "6d459bc74ba73ee4fe5cdc4655400193",
                    "jobGrowName" : "眞 트러블 슈터"
                }
                }
            }
            }, {
            "jobGrowId" : "4a1459a4fa3c7f59b6da2e43382ed0b9",
            "jobGrowName" : "스페셜리스트",
            "next" : {
                "jobGrowId" : "faf8fc6f0cd4c0f84f0035e23bd4fa19",
                "jobGrowName" : "코어 마스터",
                "next" : {
                "jobGrowId" : "64bbd751c0fcae0e74758b871a2296c1",
                "jobGrowName" : "패스파인더",
                "next" : {
                    "jobGrowId" : "c9b492038ee3ca8d27d7004cf58d59f3",
                    "jobGrowName" : "眞 스페셜리스트"
                }
                }
            }
            } ]
        }, {
            "jobId" : "b9cb48777665de22c006fabaf9a560b3",
            "jobName" : "아처",
            "rows" : [ {
            "jobGrowId" : "df3870efe8e8754011cd12fa03cd275f",
            "jobGrowName" : "뮤즈",
            "next" : {
                "jobGrowId" : "4ec01f4ae3728c080f28a72213b6df10",
                "jobGrowName" : "헤드라이너",
                "next" : {
                "jobGrowId" : "80ec67d0356defa46a989914caca5820",
                "jobGrowName" : "트렌드세터",
                "next" : {
                    "jobGrowId" : "37495b941da3b1661bc900e68ef3b2c6",
                    "jobGrowName" : "眞 뮤즈"
                }
                }
            }
            }, {
            "jobGrowId" : "1ea78ae210f681a799feb4403a5c1e85",
            "jobGrowName" : "트래블러",
            "next" : {
                "jobGrowId" : "981d675414f83b926db5f54e5347eec0",
                "jobGrowName" : "드리머",
                "next" : {
                "jobGrowId" : "8022a99e29b5e4845985fe1d79a233f3",
                "jobGrowName" : "셀레스티얼",
                "next" : {
                    "jobGrowId" : "618326026de1a1f1cfba5dbd0b8396e7",
                    "jobGrowName" : "眞 트래블러"
                }
                }
            }
            } ]
        } ]
    
}

